...
<%
    ...
    try {
        ...
        int j = st.executeUpdate("update request set dkey='" + filekey + "' where file_name='" + fname + "' AND user='" + mail + "'");
        if (j != 0) {
            Mail m= new Mail();
             String msg ="File Name : "+fname+"\nDecryption Key :"+filekey;
            m.secretMail(msg, fname, mail);
            response.sendRedirect("cspviewrequests.jsp?msg=Key_Sent");
        } else {
            response.sendRedirect("cspviewrequests.jsp?msg1=Issued_failed");
        }
