...
<%
   ...
   // int i = r.nextInt(10000 - 5000) + 5000;
    ...
    try {
        ...
        int j = st.executeUpdate("insert into keyrequest1(file_name,user,owner, status,dkey) values('" + fname + "','" + umail + "','" + email + "','Waiting','00000')");
        if (j != 0) {
            //Mail m= new Mail();
           // m.secretMail(msg, fname, umail);
            response.sendRedirect("viewwriteresponse.jsp?msg=FileKey_Request_Sent");
        } else {
            response.sendRedirect("viewwriteresponse.jsp?msg1=FileKey_Request_Failed");
        }
